...
class TestCalculatedSummaryContext(TestStandardSummaryContext):
    ...
    def setUp(self):
        ...
        answers = [
            ...
            {'answer_instance': 0, 'group_instance': 0, 'answer_id': 'second-number-answer-not-in-total', 'value': 3},
            {'answer_instance': 0, 'group_instance': 0, 'answer_id': 'third-number-answer-not-in-total', 'value': 5},
            ...
            {'answer_instance': 0, 'group_instance': 0, 'answer_id': 'fifth-number-answer', 'value': 6}
        ]
    ...
    def test_build_view_context_for_calculated_summary(self):
        csrf_token = None
        variables = None

        current_location = Location(
            block_id='total-playback',
            group_id='group',
            group_instance=0,
        )
        ...
        self.assertEqual(context_summary['title'],
                         'We calculate the total of values entered to be £13.00. Is this correct?')
        ...
        self.assertEqual(context_summary['calculated_question']['answers'][0]['value'], '£13.00')
